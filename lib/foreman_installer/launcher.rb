...
module ForemanInstaller
  class ForemanKafoLauncher

    def initialize(app_name = 'Foreman')
      @app_name = app_name
    end
    ...
    def run()
      ...
      Kafo::KafoConfigure.app_option '--reset-foreman-db', :flag,
                                     "Drop $s database(s) first? You will lose all data! Unfortunately we\n" % @app_name + 
      ...
      Kafo::KafoConfigure.hooking.register_pre(:reset_db) do |kafo|
        if kafo.config.app[:reset_foreman_db] && !kafo.config.app[:noop]
          ...
          if $?.success?
            ...
            Kafo::KafoConfigure.logger.warn '#{@app_name} not installed yet, can not drop database!'
          end
        end
      end
      ...
      if [0,2].include? @result.exit_code
        ...
        if module_enabled? 'foreman'
          say "  * <%= color('#{@app_name}', :info) %> is running at <%= color('#{get_param('foreman','foreman_url')}', :info) %>"
